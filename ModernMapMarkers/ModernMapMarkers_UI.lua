...
local function CreateFindPanel(anchorFrame)
	...
	local panelStrata = IsAddOnLoaded("Cartographer") and WorldMapFrame:GetFrameStrata() or "DIALOG"
...
local function PositionDropdowns()
	...
	local haspfQuest		= IsAddOnLoaded("pfQuest-Reforged")
	...
	if windowed and hasElvUI then
		...
		elseif hasQuestie or haspfQuest or hasWDM then
	...
	elseif hasElvUISmallerMap then
		...
		elseif (hasMapster and hasQuestie or haspfQuest) or hasWDM then
	...
	elseif hasElvUI then
		...
		if hasPfQuest then
			MMMFilterDropdown:SetPoint("TOPRIGHT", pfQuestMapDropdown, "BOTTOMRIGHT", 0, -2)
		elseif ((hasMapster or hasCartographer) and hasQuestie or haspfQuest) or hasWDM then
...
local function CreateDropdowns()
	...
	local function ApplyDropdownStrata()
		...
		if findPanel then
			findPanel:SetFrameStrata("TOOLTIP")
			findPanel:SetFrameLevel(100)
		end
	end

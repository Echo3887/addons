-- init.lua
-- Runs after DataTexts.lua (ensure TOC order: DataTexts.lua then init.lua then modules/Load_Modules.xml).
-- Exposes lightweight helpers for modules and (optionally) runs any pending registrations.

print("|cff00ff00[DataTexts:init]|r init.lua running")

local core = EWTweaker and EWTweaker.GetCore and EWTweaker.GetCore()
if not core then
    print("|cff00ff00[DataTexts:init]|r EWTweaker core NOT present")
    return
end

print("|cff00ff00[DataTexts:init]|r EWTweaker core present")
